...
include("../connection.php");
    ...
    $received = mysqli_query($conn, "SELECT * FROM received"); 

    $received2 = mysqli_query($conn, "SELECT * FROM received");
    $rowss = mysqli_fetch_assoc($received2);
    if(isset($_SESSION['received_id'])){
        $_SESSION['received_id'] = $rowss['received_id'];
    }
    ...
    if(isset($_GET['sirch'])){
        ...
        $received = mysqli_query($conn, "SELECT * FROM received WHERE reference_id = '$search'");
    }

    if(isset($_POST['seeAll'])){
        header("Location: adminQueuesReceive.php");
    }   
            ...
            <div class="header">
                ...
                <div class="queuesOrders" id="queuesOrders">
                    <div class="queuesOrders-content">
                        <h1>Queues</h1>
                        <p>Queues <span>></span> Received</p>
                    </div>
                    <form action="" method="get">
                        <input type="text" name="search" id="" placeholder="Search reference id">
                        <button name="sirch">Search</button>
                        <button name="seeAll">See All</button>
                    </form>
                    <center>
                        <?php
                                while ($row3 = mysqli_fetch_assoc($received)) {
                            ?>
                                <div>
